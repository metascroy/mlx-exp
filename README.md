...
* src/program_json_loader.hpp: loader to convert program.json to program object in src/program.hpp.  This file is auto-generated by the python script generate_from_schema.py based on ops_schema.py
...
Once this runs, you can run the program in the interpreter by running `sh build_and_run_llama.sh`.
You might have to adjust `src/main_llama.cpp` to point to the right locations for the above files:
...
# End to end test with Whisper

1. Export whisper model with `python export_whisper.py`.  This will create 3 whisper programs: encoder, kv-projector, decoder.  You can modify export settings (dtype, quantization) by editing DEFAULT_EXPORT_KWARGS inside `export_whisper.py`.
