...
inline DTypeId parse_dtype(const std::string& s) {
  ...
  if (s == "DTypeId.i8") return DTypeId::i8;
...
inline Tid parse_tid_obj(const nlohmann::json& j) {
  // Handle both {"tid": N} and [{"tid": N}] formats
  if (j.is_array() && j.size() == 1) {
    const auto& elem = j[0];
    if (!elem.is_object() || !elem.contains("tid"))
      throw std::runtime_error("parse_tid_obj: expected [{\"tid\": N}]");
    Tid t{};
    t.idx = elem.at("tid").get<uint32_t>();
    return t;
  }
  ...
  for (const auto& jinstr : jcode) {
    ...
    // ========== AUTO-GENERATED OP PARSING ==========
      if (op == "NOOP") {
        ...
        P.code.push_back(make_ITEM_INT(std::move(n)));
      }
      else if (op == "EXPAND_DIMS") {
        ExpandDimsNode n;
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        n.out = detail::parse_tid_obj(jinstr.at("out"));
        n.axis = detail::parse_int_strict(jinstr.at("axis"));
        P.code.push_back(make_EXPAND_DIMS(std::move(n)));
      }
      else if (op == "TILE") {
        TileNode n;
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        n.out = detail::parse_tid_obj(jinstr.at("out"));
        n.reps = jinstr.at("reps").get<std::vector<int>>();
        P.code.push_back(make_TILE(std::move(n)));
      }
      else if (op == "TAKE_ALONG_AXIS") {
        TakeAlongAxisNode n;
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        n.indices = detail::parse_tid_obj(jinstr.at("indices"));
        n.out = detail::parse_tid_obj(jinstr.at("out"));
        n.axis = detail::parse_int_strict(jinstr.at("axis"));
        P.code.push_back(make_TAKE_ALONG_AXIS(std::move(n)));
      }
      else if (op == "RMS_NORM") {
        RMSNormNode n;
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        ...
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        n.out = detail::parse_tid_obj(jinstr.at("out"));
        if (jinstr.contains("weight") && !jinstr.at("weight").is_null()) {
          n.weight = detail::parse_tid_obj(jinstr.at("weight"));
        } else {
          n.weight = std::nullopt;
        }
        if (jinstr.contains("bias") && !jinstr.at("bias").is_null()) {
          n.bias = detail::parse_tid_obj(jinstr.at("bias"));
        } else {
        ...
        n.head_dim = detail::parse_int_strict(jinstr.at("head_dim"));
        ...
        if (jinstr.contains("dtype") && !jinstr.at("dtype").is_null()) {
        ...
        n.x = detail::parse_tid_obj(jinstr.at("x"));
        n.out = detail::parse_tid_obj(jinstr.at("out"));
        auto dtype_json = jinstr.at("dtype");
        std::string dtype_string = "DTypeId.i32";
        if (!dtype_json.is_null()) {
          dtype_string = dtype_json.get<std::string>();
        }
        ...
        n.group_size = detail::parse_int_strict(jinstr.at("group_size"));
        n.bits = detail::parse_int_strict(jinstr.at("bits"));
        n.mode = jinstr.at("mode").get<std::string>();
        auto out_dtype_json = jinstr.at("out_dtype");
        std::string out_dtype_string = "DTypeId.i32";
        if (!out_dtype_json.is_null()) {
          out_dtype_string = out_dtype_json.get<std::string>();
        }
        n.out_dtype = detail::parse_dtype(out_dtype_string);
